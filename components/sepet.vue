<template>
  <div>
    <h1 class="text-2xl font-bold text-gray-800 mb-4">Sepetim</h1>
    <div v-if="cartItems.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
      <div
    ...
    <div v-else class="text-center text-gray-500 text-lg font-semibold py-8">
      Sepetiniz boş. Ürün eklemek için alışverişe başlayın.
    </div>

    <!-- Satın Al Bölümü -->
    <div v-if="cartItems.length" class="p-4 mt-6 border-t border-gray-300">
      <h2 class="text-xl font-semibold text-gray-800 mb-2">Toplam: ₺{{ totalPrice }}</h2>
      <button @click="checkout" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded">
        Satın Al
      </button>
    </div>
  </div>
